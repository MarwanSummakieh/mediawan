...
function renderSeasonSelect() {
  ...
  renderSeasonTabs("d-seasonList", {
    ...
    options: seasons.map((s, i) => ({ value: s.anilistId, label: `Season ${i + 1}`, sub: s.title + (s.year ? ` (${s.year})` : "") })),
    onPick: (v) => {
      const id = Number(v);
...
const SUB_SIZES = [
  { id: "s", label: "Small", f: 0.03 },
  { id: "m", label: "Medium", f: 0.038 },
  { id: "l", label: "Large", f: 0.048 },
  { id: "xl", label: "Extra large", f: 0.06 },
  { id: "xxl", label: "Huge", f: 0.075 }
];
const SUB_COLORS = [
...
const SUB_DEFAULTS = { size: "m", color: "white", bg: "black", bgOpacity: 0.75, pos: 6, align: "center" };
...
const SubStyle = {
  ...
  _preview: false,
  _picH: 0,
  // height of the video's picture, the unit font size is in
  init() {
  ...
  apply(fromClick) {
    ...
    if (fromClick) setTimeout(menus, 0);
    else menus();
  },
  // ---- the track ----
  attach(trackEl) {
  ...
  detach() {
    if (this.trackEl && this._onCue) {
      try {
        this.trackEl.track.removeEventListener("cuechange", this._onCue);
      } catch (e) {
      }
    }
    this.trackEl = null;
  ...
  _isTop(cue) {
    const line = cue.line;
    if (line === void 0 || line === null || line === "auto") return false;
    const n = Number(line);
  ...
  _cueBox(content) {
    ...
    box.style.textShadow = bgHex && s.bgOpacity >= 0.5 ? "none" : SUB_EDGE;
  ...
  _cueNode(cue) {
    let frag = null;
    try {
      if (cue.getCueAsHTML) frag = cue.getCueAsHTML();
    } catch (e) {
      frag = null;
    }
  ...
  _stack(boxes, edge) {
    ...
    el.style.alignItems = this.s.align === "left" ? "flex-start" : this.s.align === "right" ? "flex-end" : "center";
  ...
  _rows(host, items, onPick) {
    const el = $(host);
    if (!el) return;
    const chip = (c) => c === void 0 ? "" : c ? `<i class="sub-chip" style="background:${c}"></i>` : `<i class="sub-chip none"></i>`;
    el.innerHTML = items.map((it, i) => `<button class="p-menu-item ${it.on ? "active" : ""}" data-i="${i}"><span>${chip(it.chip)}${esc(it.label)}</span></button>`).join("");
  ...
  buildMenus() {
    ...
    const bgVal = bg.hex ? `${bg.label} \xB7 ${Math.round(s.bgOpacity * 100)}%` : "None";
    this._val("#valSubStyle", size.label);
    const list = $("#subStyleList");
    if (list) {
      list.innerHTML = [
        ["subsize", "Text size", size.label],
        ["subpos", "Position", `${s.pos}% \xB7 ${align.label}`],
        ["subcolor", "Text colour", color.label],
        ["subbg", "Background", bgVal]
      ].map(
        ([to, label, val]) => `<button class="p-menu-row" data-to="${to}"><span>${label}</span><span class="p-menu-val">${esc(val)}</span></button>`
    ...
    this._rows(
      "#subSizeList",
      SUB_SIZES.map((x) => ({ ...x, on: x.id === s.size })),
      (x) => this.set("size", x.id)
    );
    this._rows(
      "#subColorList",
      SUB_COLORS.map((x) => ({ ...x, chip: x.hex, on: x.id === s.color })),
    ...
    if (pos) {
      ...
      this._rows(
        "#subAlignList",
        SUB_ALIGNS.map((x) => ({ ...x, on: x.id === s.align })),
    ...
    if (bgList) {
      ...
      this._rows(
        "#subBgColorList",
        SUB_BGS.map((x) => ({ ...x, chip: x.hex, on: x.id === s.bg })),
        (x) => this.set("bg", x.id)
      );
      if (bg.hex) this._rows(
        "#subBgOpacityList",
...
const Player = {
  ...
  buildCcMenu() {
    ...
    $("#ccList").innerHTML = rows.join("") + sync + style + (((_a = this.subsAvail) == null ? void 0 : _a.length) ? "" : `<div class="p-menu-empty">No subtitles found for this one yet.</div>`);
...
const SEASON_PICK = /* @__PURE__ */ new Map();
...
async function loadSeasonEps(id, season) {
  ...
  setSeasonTab("m-seasonList", season);
