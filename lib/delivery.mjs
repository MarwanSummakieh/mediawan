...
// `replaces` is the id of the session the viewer is leaving behind — a
// settings change re-requesting the same title. It travels down to the session
// layer, which retires it before opening the new one.
export async function deliver(stream, { local = false, seekSec = 0, title = null, mode = null, audioIndex = null, maxHeight = null, replaces = null } = {}) {
  ...
  return session(source, key, {
    local, seekSec, mode, audioIndex, maxHeight, replaces, fromCache,
    expectedBytes: fromCache ? cached.bytes : expectedBytes,
    remoteUrl: stream.url,
  });
}

async function session(filePath, key, { local, seekSec, expectedBytes = 0, mode = null, audioIndex = null, maxHeight = null, replaces = null, fromCache = false, remoteUrl = null }) {
  try {
    const s = await transcode.startSession({
      filePath, identity: key, cacheKey: fromCache ? key : null,
      local, seekSec, expectedBytes, mode, audioIndex, maxHeight, replaces,
    });
    if (s.mode === "direct") {
    ...
    // error dialog.
    //
    // The probe rides along when the failure happened after the file was read.
    // Whether the raw release is a usable fallback depends entirely on what is
    // inside it, and this is the only place that knows.
    return { kind: "unavailable", error: e.message, key, probe: e.probe || null };
  }
}
