...
export async function deliver(stream, { local = false, seekSec = 0, title = null, mode = null, audioIndex = null } = {}) {
  ...
  return session(source, key, {
    local, seekSec, mode, audioIndex, fromCache,
    expectedBytes: fromCache ? cached.bytes : expectedBytes,
    remoteUrl: stream.url,
  });
}

async function session(filePath, key, { local, seekSec, expectedBytes = 0, mode = null, audioIndex = null, fromCache = false, remoteUrl = null }) {
  try {
    const s = await transcode.startSession({
      filePath, identity: key, cacheKey: fromCache ? key : null,
      local, seekSec, expectedBytes, mode, audioIndex,
    });
    if (s.mode === "direct") {
