...
// The resolution dial, normalized. Anything unset, unparseable or absurd means
// "no opinion", and the planner then applies its own rule. Clamped rather than
// rejected at the top so a client asking for more than the file holds simply
// gets the file, and floored at 144 because below that the number is a typo
// rather than a preference.
export function normalizeHeight(h) {
  const n = Math.floor(Number(h) || 0);
  if (!Number.isFinite(n) || n < 144) return null;
  return Math.min(4320, n);
}
...
export function planDelivery(probe, { local, targetMbps = config.transcode.remoteMbps, mode: audioMode = null, audioIndex = null, maxHeight = null } = {}) {
  const v = probe.video;
  // `maxHeight` is the viewer's RESOLUTION dial, and the only thing on this
  // path they set by hand. Everything else here is inferred — what the client
  // can decode, what the uplink can carry — and the inference had exactly one
  // lever on picture size: "a capped encode of a >1080p source is scaled to
  // 1080p". That rule is right on average and wrong whenever the viewer
  // disagrees with it, in either direction: a 4K source over the tunnel was
  // pinned to 1080p however good the connection was, and nobody could ask for
  // LESS than the source to spend a small budget on a clean small picture
  // instead of a smeared big one. When the dial is set it REPLACES the
  // automatic rule rather than arguing with it.
  const cap = normalizeHeight(maxHeight);
  ...
  // Does the dial actually ask for anything? Only a cap BELOW the source is
  // work; asking for 1080p from a 1080p file is asking for the file.
  const srcHeight = v.height || 0;
  const downscale = cap && srcHeight && srcHeight > cap ? cap : null;

  if (local) {
    // On the LAN there is bandwidth to spare, so quality is never traded away.
    // The only question is whether the client can open the container — unless
    // the viewer has asked for a smaller picture, which is the one reason to
    // re-encode a perfectly deliverable LAN stream. Uncapped still: a 720p pick
    // on the LAN means a 720p picture at whatever bitrate it deserves, not a
    // 720p picture squeezed through the remote budget.
    if (downscale) {
      return {
        ...common,
        mode: "encode",
        reason: `LAN client, scaled to ${downscale}p on request (source ${srcHeight}p)${langNote}`,
        copyVideo: false,
        targetMbps: null,
        scaleHeight: downscale,
    ...
    if (codecsFine && mp4ish && singleTrack)
      return { ...common, mode: "direct", reason: `browser-safe codecs in an MP4 container${langNote}`, outputHeight: srcHeight || null };
    if (codecsFine)
      return { ...common, mode: "remux", reason: `repackaging ${probe.container} without re-encoding${audioMode ? `, ${audioMode} track` : ""}${langNote}`, outputHeight: srcHeight || null };
  ...
  // A requested downscale is a re-encode by definition, so it disqualifies the
  // copy paths the same way an undecodable codec does.
  const videoOk = v.browserSafe && !overBudget && !downscale;
  if (videoOk && (!a || a.browserSafe)) {
    return { ...common, mode: "remux", reason: `already ${probe.mbps?.toFixed(1)} Mbps, under the ${targetMbps} Mbps remote budget${langNote}`, outputHeight: srcHeight || null };
  }
  ...
  // WHICH HEIGHT COMES OUT.
  //
  // An explicit dial wins outright — including upward: asking for 2160p over
  // the tunnel keeps 2160p, because "the tunnel is small" is an argument about
  // bitrate, and the viewer has just said they would rather spend it on pixels.
  // With no dial the old rule stands: a capped encode is worth far more at
  // 1080p than at 4K, where the same bits spread over four times the pixels and
  // the result is mud.
  const scaleHeight = cap ? downscale : autoScale(srcHeight, videoOk ? null : targetMbps);
  return {
    ...common,
    mode: "encode",
    reason: videoOk
      ? `remote client: video copied, ${a?.codec || "audio"} needs conversion${langNote}`
      : `remote client: ${probe.mbps ? `${probe.mbps.toFixed(1)} Mbps` : "unknown bitrate"} → ${targetMbps} Mbps${scaleHeight ? `, ${scaleHeight}p` : ""}${langNote}`,
    copyVideo: videoOk,
    targetMbps: videoOk ? null : targetMbps,
    scaleHeight,
    outputHeight: scaleHeight || srcHeight || null,
  };
}

// The automatic picture size, used only when the viewer has not set the dial:
// spend a bitrate cap on 1080p, never on 2160p. Uncapped encodes keep the
// source resolution — there the codec, not the size, was the problem.
function autoScale(srcHeight, targetMbps) {
  return targetMbps && srcHeight > 1080 ? 1080 : null;
}
...
export function ffmpegArgs({ input, outDir, plan, probe, seekSec = 0, forceSoftware = false }) {
  ...
  //
  // When the video is COPIED the seek must be keyframe-exact, not time-exact.
  // An input -ss lands the demuxer on the keyframe at-or-before the target, and
  // a copied stream has no choice but to keep everything from that keyframe —
  // but accurate_seek (the default) still trims DECODED streams to the exact
  // target. Those two rules split the streams: a copy-video session resumed
  // mid-GOP shipped video from the keyframe and audio from the resume point,
  // a constant offset of up to one GOP that players render as audio desync.
  // Measured on this ffmpeg build: -ss 47 into a 4s-GOP source put the first
  // video packet at t=0.083 and the first audio packet at t=3.061. This path
  // was rare while remote plays picked 4K remuxes (over budget → full encode,
  // both streams trimmed identically); the moment 1080p picks came in under
  // budget, video copy became the norm and every resume desynced.
  //
  // -noaccurate_seek starts every stream — audio, and the subtitle input below,
  // which shares these opts — at that same keyframe. Playback begins up to a
  // GOP before the requested point, which is how Plex and Jellyfin resume too;
  // a few seconds of replay is the correct price for aligned tracks. Full
  // encodes keep the accurate seek: everything decodes, so everything trims to
  // the same instant and the resume point stays exact.
  //
  // The flag has no purchase on an ENCODED video stream: measured on ffmpeg
  // N-124716, an -ss 10 encode started its first video packet at the exact
  // target with or without it. So a session that encodes video while copying
  // audio still opens with the copied track's keyframe lead-in — a couple of
  // seconds of sound before the first frame. The tracks stay correctly timed
  // against each other, which is why that is left alone rather than "fixed"
  // with a flag that does nothing here.
  if (seekSec > 0) {
    if (plan.mode === "remux" || plan.copyVideo) inputOpts.push("-noaccurate_seek");
    inputOpts.push("-ss", String(seekSec));
  }
  ...
  if (plan.mode === "remux" || plan.copyVideo) {
    ...
    const filters = [];
    // WHAT SIZE comes out is the planner's decision (plan.scaleHeight), not
    // this function's: it is the one picture property the viewer sets by hand,
    // and a second copy of the rule down here is a second place for the two to
    // disagree. Scaling runs before the tonemap chain so the expensive zscale
    // work touches a quarter of the pixels. -2 keeps the aspect ratio and
    // rounds the width to an even number, which H.264 requires.
    const scaleTo = plan.scaleHeight ?? null;
    if (scaleTo && (probe.video?.height || 0) > scaleTo) filters.push(`scale=-2:${scaleTo}`);
...
// Can the client be handed the RAW release and left to it?
//
// This is the same test planDelivery's `direct` branch makes, kept beside it
// for the same reason: it is the one place that decides whether an untouched
// file is playable, and two copies of that rule would disagree. It answers a
// different caller — the fallback taken when local delivery is refused
// outright — which is why it takes a probe rather than a plan.
//
// The audio half is what makes it worth asking. A release is typically an MKV
// carrying AC-3, E-AC-3 or DTS; no browser decodes any of them, so passing one
// through renders the picture and NOTHING ELSE, with nothing on screen to say
// why. Silence that looks like a bug in the app is worse than an error that
// says the transcoder was busy.
export function rawFileIsPlayable(probe) {
  if (!probe?.video?.browserSafe) return false;
  if (!/mp4|mov|m4v/i.test(probe.container || "")) return false;
  // Every track, not just the chosen one: nobody is mapping streams here, so
  // the browser plays whichever it likes.
  return (probe.audio || []).every((t) => t.browserSafe);
}
...
export async function startSession({ filePath, cacheKey, identity, local, seekSec = 0, expectedBytes = 0, mode = null, audioIndex = null, maxHeight = null, replaces = null }) {
  ...
  const plan = planDelivery(probe, { local, mode, audioIndex, maxHeight });
  ...
  // The output HEIGHT belongs in the key for the same reason the audio index
  // does: two viewers watching one file at different resolutions are two
  // encodes, and sharing them would hand the second viewer the first one's
  // picture size.
  const keyBase = `${identity || filePath}|${plan.mode}|${plan.targetMbps ?? "x"}|${plan.scaleHeight ?? "x"}|${plan.audioIndex ?? "x"}`;
  return openSession({ input: filePath, keyBase, plan, probe, cacheKey, seekSec, replaces });
}
...
async function openSession({ input, keyBase, plan, probe, cacheKey, seekSec = 0, replaces = null }) {
  ...
  // The viewer is LEAVING `replaces` for this one. Retiring it here — before
  // the capacity check, and only when it really is a different session — is the
  // same reasoning reseekSession applies to a seek: one viewer, one timeline,
  // one encoder.
  //
  // Without this, changing a setting mid-play could reliably fill the box. The
  // dial's height is part of the session key, so a resolution change always
  // opens a SECOND encoder while the first is still counted, and the
  // abandoned-session reclaim below cannot help: that session was read
  // milliseconds ago and only looks abandoned after 10s. On a 2-session box
  // that is a hard "transcoder busy" for the second change in a row — and what
  // the viewer got for it was the raw release, which for the usual MKV/AC-3
  // shape is a picture with no sound.
  //
  // It carries reseekSession's caveat too: a session is shared by everyone
  // watching the same thing the same way, so this can retire a stream a second
  // viewer is on. That trade is already made for seeks, and the alternative —
  // never releasing what a viewer has demonstrably left — is what fills a small
  // box.
  if (replaces && replaces !== id) await stopSession(replaces);
  ...
  if (sessions.size >= config.transcode.maxSessions) {
    // The probe rides along on the error. Refusing to transcode sends the
    // caller looking for a fallback, and whether the raw release is a fallback
    // at all is a question only the probe can answer — see rawFileIsPlayable.
    const busy = new TranscodeError(`transcoder busy (${sessions.size}/${config.transcode.maxSessions} sessions in active use)`);
    busy.probe = probe;
    throw busy;
  }
  ...
  const spawnFfmpeg = (forceSoftware) => {
    ...
    proc.on("exit", (code) => {
      ...
      if (!forceSoftware && /qsv|encoder|Invalid argument|no packets/i.test(session.stderr)) {
        ...
        // EMPTY THE DIRECTORY FIRST. The retry runs the same command in the
        // same place, and append_list means ffmpeg reads the playlist the dead
        // attempt left behind and CONTINUES it: the software segments are
        // appended after the QSV ones while init.mp4 is overwritten with the
        // new encoder's, leaving a playlist whose first stretch no longer
        // matches its init segment. That is the failure sweepSessionRoot exists
        // to prevent for a killed process — the retry reproduced it inside one
        // live session, and reproduced it exactly where QSV is most likely to
        // refuse in the first place: a downscale to a size the viewer picked.
        // Re-checked either side of the await: stopSession removes the whole
        // directory, so a session retired while this is in flight must not have
        // it rebuilt underneath.
        if (sessions.get(id) !== session) return;
        fsp.rm(outDir, { recursive: true, force: true })
          .then(() => fsp.mkdir(outDir, { recursive: true }))
          .then(() => { if (sessions.get(id) === session) spawnFfmpeg(true); })
          .catch((err) => {
            session.exited = true;
            session.exitCode = code;
            console.warn(`  [transcode] ${id}: could not clear ${outDir} for the software retry — ${err.message}`);
          });
        return;
      }
      session.exited = true;
