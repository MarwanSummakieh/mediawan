...
// Is this the SERVICE refusing the ACCOUNT, rather than one release being
// unavailable? The distinction is the difference between "try another release"
// and "nothing will play until you fix your subscription", and getting it
// wrong is expensive in both directions.
//
// Real-Debrid's own error codes are the reliable signal: 8 is a dead token,
// 9 is permission denied — which is exactly what /torrents/addMagnet answers
// once premium lapses, because a free account may not add torrents at all.
// Neither message contains the word "premium", which is why the caller's
// string match for it never caught the one failure that takes the entire
// debrid tier down, and instead reported it once per release as if some
// particular rip were at fault.
export const accountFault = async (e, { probe = userInfo } = {}) => {
  const msg = e?.message || "";
  const looksAccount = e?.code === 8 || e?.code === 9 || e?.status === 401 || e?.status === 403
    || /bad[_ ]token|permission_denied/i.test(msg);
  if (!looksAccount) return /premium|parcel/i.test(msg) ? msg : null;

  // Do NOT take a 403 at face value. Real-Debrid answers permission_denied for
  // more than a dead subscription — it did so once here while premium was
  // demonstrably live — and since this verdict ABORTS the search after a
  // single release, a false positive is worse than the bug it replaced:
  // one transient refusal would condemn a hundred perfectly good releases and
  // send the viewer off to renew a subscription they already hold.
  //
  // /user is the authority and costs one cheap call on a path that has already
  // failed. Ask it rather than inferring.
  let u = null;
  try { u = await probe(); } catch { u = null; }
  if (u?.type === "premium" && u.premium > 0) return null;   // account is fine — this was something else
  if (u) return "premium has lapsed — a free account cannot add torrents, so NO release can play until it is renewed";

  // /user itself would not answer. A refused token explains both that and the
  // original error; anything else is an outage, and an outage must stay an
  // ordinary API error so the loop keeps trying.
  if (e?.code === 8 || e?.status === 401 || /bad[_ ]token/i.test(msg))
    return "the API token was rejected (expired or revoked) — put a fresh REAL_DEBRID_TOKEN in .env";
  return null;
};
