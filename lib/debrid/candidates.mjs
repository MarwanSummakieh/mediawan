...
export function spreadByQuality(list) {
  ...
  // Band order follows the LIST, not the numbers. A Map iterates in insertion
  // order, so the band the caller ranked first stays first — which keeps the
  // top pick unchanged for an ordinary ranked list (it already leads with the
  // best release) while no longer overriding a caller that deliberately
  // ordered things differently.
  //
  // Sorting numerically instead silently undid preferTranscodeFriendly: a
  // REMOTE viewer gets a ~6 Mbps encode, so the pool is deliberately reordered
  // to put 1080p ahead of 4K, and this put the 4K back at the front every
  // time. The encoder was then handed a 71.6 Mbps HDR remux to downscale, and
  // the first HLS segment took 29 SECONDS to appear. It also overruled the
  // ranker in general: a 1080p REMUX that outranks a 2160p WEB-DL was demoted
  // beneath it purely for having a smaller number in its name.
...
// A backend that cannot classify its own errors still gets this: the shapes
// that say so in words. Real-Debrid's lapsed-subscription error does NOT —
// it is a bare "403 permission_denied" — which is why backends may supply a
// precise accountFault of their own. This stays as the floor so a service
// without one never silently loses the protection.
const genericAccountFault = (e) => (/premium|parcel/i.test(e?.message || "") ? e.message : null);
...
export async function streamFirstCached(candidates, {
  ...
  // Services that refused the ACCOUNT. Kept apart from ordinary errors because
  // the answer is a different KIND of thing: not "try another release" but
  // "renew the subscription". Everything downstream — status code, player
  // message, whether a "choose a different release" button is even offered —
  // hangs off telling those two apart.
  const acctFaults = [];
  for (const backend of list) {
    ...
    // Set when the SERVICE refused the ACCOUNT. Distinct from apiErrors: no
    // number of further releases can change the answer, and the viewer needs
    // to be told to renew a subscription, not to pick a different rip.
    let acctFault = null;
    ...
    for (const c of spreadByQuality(pool).slice(0, max)) { // cap attempts so a play can't hang
      ...
      try {
        ...
        // Account-level failures (lapsed subscription, dead token, locked
        // "parcel") won't fix themselves by trying more torrents on THIS
        // service — but another backend may be perfectly healthy, so move on
        // rather than give up. Asking the backend beats matching on prose:
        // Real-Debrid says "403 permission_denied" when premium runs out, and
        // the old /premium|parcel/ test read that as an ordinary API error —
        // so the one fault that stops EVERY release was retried against a
        // dozen of them and then blamed on the last one.
        // await: a backend may need to ask the service whether the ACCOUNT is
        // really the problem before condemning every remaining release.
        acctFault = (await backend.accountFault?.(e)) || genericAccountFault(e);
        if (acctFault) { lastErr = e; break; }
        apiErrors++; lastErr = e;
      }
    }
    ...
    // One cause, stated once, and NOT counted as a genuine cache answer: a
    // service that never looked cannot have told us the film is absent.
    if (acctFault) { acctFaults.push(backend.label); notes.push(`${backend.label}: ${acctFault}`); continue; }
  ...
  return {
    // "debrid-account" only when NOTHING looked properly: if any backend gave a
    // real cache answer the catalogue genuinely lacks the film, and a lapsed
    // account elsewhere is beside the point.
    error: sawRealAnswer ? "no cached release" : (acctFaults.length ? "debrid-account" : "debrid-error"),
